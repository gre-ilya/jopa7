...
struct GeoPoint {        // географическая точка
    ...
    bool    hasAltitude;
    QDateTime time;      // время создания точки (UTC); isValid()==false, если
                         // в файле времени не было — тогда и не записывается
};
...
- **Время точек** читается из GPX (`<time>`) и GDB и пишется обратно в оба
  формата (`GeoPoint::time`, UTC; GDB хранит целые секунды). GeoJSON время
  точек не хранит.
- **Треки (`trk`) при чтении игнорируются** — в `GeoData` попадают только
  путевые точки и маршруты (`rte`), поэтому точки записанных треков не
  засоряют общий пул. Маршрут, сохранённый в GeoJSON как `LineString`,
  при чтении возвращается маршрутом.
